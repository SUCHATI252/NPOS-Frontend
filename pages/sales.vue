<template>
  <v-row class="fill-height" no-gutters>
    <v-col cols="8" class="pr-2">
      <v-card class="overflow-y-auto fill-height" width="100%">
        <v-toolbar flat dense>
          ...
          >
            <template #activator="{ on, attrs }">
              <v-app-bar-nav-icon aria-label="nav-icon" v-bind="attrs" v-on="on" />
            </template>
            <v-list>
              <v-divider />
              <v-list-item :to="localePath('/dashboards')">
                <v-list-item-icon>
              ...
              <v-divider />
              <v-list-item @click="$refs.logout.open()">
                <v-list-item-icon>
          ...
          <span>Hi</span>&nbsp;<span>{{ $auth.user.name }}</span>
          ...
          <v-divider class="mx-3" vertical />
          <v-btn icon aria-label="FullScreen" @click="toggleFullScreen">
                  ...
                  >
                    <v-hover v-slot="{ hover }">
                      <v-card
                        ...
                        @click="get_product(product.pd_code)"
                      >
                        <v-img
                          :src="`${$config.SERVER_URL}/products/Image/${product.pd_image}`"
                          aspect-ratio="1"
                          height="80px"
                        />
                        <v-card-text>
    ...
    <v-col cols="4" class="pl-2">
      <v-card width="100%" class="d-flex flex-column pa-3 fill-height">
          ...
          <v-form ref="form_barcode" @submit.prevent="find_barcode">
            ...
            >
              <template #append>
                ...
                <v-icon aria-label="find_barcode" @click="find_barcode">
                  mdi-magnify
                </v-icon>
              </template>
            </v-text-field>
        ...
        <v-card class="pa-2 overflow-y-auto" outlined tile max-height="45vh">
              ...
              <thead>
                <tr>
                  ...
                  <th class="text-center">
                    <v-icon aria-label="delete-forever" :disabled="item_disabled" :color="listsale.length>0?'error':''" @click="Clear_All">
              ...
              <tbody>
                <tr v-for="(item, index) in listsale" :key="index">
                  ...
                  <td class="text-center">
                    <v-icon aria-label="delete" :disabled="item_disabled" small color="error" @click="removeItem(item)">
                      mdi-close
                    </v-icon>
                  </td>
                </tr>
        ...
        <v-card v-if="!go_to" class="d-flex justify-space-between py-2 px-5" outlined tile>
          ...
          <div class="text-right">
            <span class="font-weight-black text-h4 error--text">
        ...
        <v-card v-else flat class="d-flex justify-space-between pa-2">
          <div>
            <v-card class="pa-3 text-center mr-1" color="primary" dark @click="printslip">
              <span class="headline font-weight-black">
    ...
    <logout ref="logout" />
    <print-slip ref="printslip" :data="listsale" />
  </v-row>
</template>
...
export default {
  ...
  watch: {
    ...
    cash (val) {
      if (parseInt(val) > 1000) {
        this.cash_msg = 'Not 4 length 1000'
      } else {
        this.cash_msg = ''
      }
    }
    ...
    //       if (event.key === 'Delete') { return this.cash_clear() }
    //       if (event.key === 'Backspace') { return this.dropcash() }
    //       if (event.key === 'Enter') { return this.Get_cash() }
    //     })
    //   }
    // }
  },
  ...
  methods: {
    ...
    async get_product (code) {
      await this.$axios.$put(`/products/find_product/${code}`).then((res) => {
        if (res.outstock) {
          ...
          for (let i = 0; i < this.listsale.length; i++) {
            if (this.listsale[i].barcode === res.product.pd_code) {
              if (this.listsale[i].qty === Number(res.product.stock)) {
                this.$store.commit('SET_ERROR', 'ERROR stock')
                found = true
                break
              }
              this.listsale[i].qty++
          ...
          if (!found) {
            this.listsale.push({
              p_id: res.product.pd_id,
              barcode: res.product.pd_code,
              name: res.product.pd_name,
              qty: 1,
              price: res.product.pd_sale
            })
          }
        } else {
          this.errormessages = 'errormessages'
    ...
    addcash (value) {
      switch (value) {
        case '10':
          return this.setcash('10')
        case '20':
          return this.setcash('20')
        case '50':
          return this.setcash('50')
        case '100':
          return this.setcash('100')
        case '500':
          return this.setcash('500')
        case '1000':
          return this.setcash('1000')
        default:
          if (this.cash === '0' ||
            this.cash === '10' ||
            this.cash === '20' ||
          ...
          return this.setcash(this.cash + value)
      }
    },
    setcash (value) {
      if (this.cash.length < 4 || this.cash === '1000') {
        this.cash = value
        this.cash_msg = ''
        this.$refs.cash.focus()
      }
    ...
    async Save_sale_Order () {
      ...
      await this.$axios.$post('/sales/save_sale', Data).then((ress) => {
        if (ress.success) {
          for (let i = 0; i < this.listsale.length; i++) {
            ...
            this.$axios.$post('/sales/save_sale_list', Datalist).then((res) => {
              if (res.success) {
                this.payment_dialog = false
    ...
    async customer_pay_submit () {
      if (this.$refs.form_customer.validate()) {
        const Data = this.FormData({ cus_code: this.cus_code, total: this.total })
        await this.$axios.$post('/customers/find_sale_customer', Data).then((res) => {
          if (res.success) {
            this.cus_id = res.id
            this.cash = this.total
            this.alertcustomer = false
            this.Save_sale_Order()
          } else if (res.notactivate) {
            this.$store.commit('SET_ERROR', 'This ID NOT Activate')
